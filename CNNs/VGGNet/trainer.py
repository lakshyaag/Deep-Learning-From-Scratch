...
config = Config(BATCH_SIZE=8, N_EPOCHS=10)
...
def evaluate_model(model: VGGNet, run_id=None):
    model.eval()
...
def predict(model: VGGNet, data: torch.Tensor, true_labels: list[str]):
    model.eval()
    with torch.inference_mode():
...
if __name__ == "__main__":
    train_loader, val_loader, test_loader, true_labels = prepare_data_loaders(
        config.BATCH_SIZE, train_split=0.6
    )

    model = VGGNet(n_classes=config.N_CLASSES, debug=False).to(device)
